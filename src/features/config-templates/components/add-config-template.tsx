...
import { useCreateConfigTemplateMutation } from "../services";
...
interface AddConfigTemplateSheetProps {
  triggerButton?: ReactNode;
  onConfigTemplateAdded?: (values: Record<string, unknown>) => void;
}
...
export const AddConfigTemplateSheet = ({ triggerButton, onConfigTemplateAdded }: AddConfigTemplateSheetProps) => {
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [createConfigTemplate] = useCreateConfigTemplateMutation();

  const form = useForm<ConfigTemplateFormValues>({
    resolver: zodResolver(configTemplateSchema),
    defaultValues: {
      name: "",
      estimation_mode: ESTIMATION_MODES.HOURLY,
  ...
  const { fields, append, remove } = useFieldArray({ control: form.control, name: "service_roles" });

  const handleSubmit = async (values: ConfigTemplateFormValues) => {
    try {
      setIsLoading(true);
      await createConfigTemplate(values as Record<string, unknown>);
      if (onConfigTemplateAdded) onConfigTemplateAdded(values as Record<string, unknown>);
      toast.success("Configuration template added successfully");
      setOpen(false);
      form.reset();
    } catch (e) {
      console.error("Error adding configuration template:", e);
      toast.error("Failed to add configuration template");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="sm:max-w-md md:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Add Configuration Template</SheetTitle>
          <SheetDescription>Create a new configuration template for your application.</SheetDescription>
        </SheetHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 py-4">
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g., Mobile App Development Template" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="Add a description" className="resize-none" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="buffer_percentage" render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Additional buffer as percentage (0-100)</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="hours_per_day" render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Working hours in a day (1-24)</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
            ...
            <SheetFooter className="pt-4 sm:justify-between">
              ...
              <Button type="submit" disabled={isLoading}>
                {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating...</> : "Create Template"}
              </Button>
            </SheetFooter>
          </form>
        </Form>
